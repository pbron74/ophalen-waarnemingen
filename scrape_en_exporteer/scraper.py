...
import pandas as pd
from config import DATA_DIR, GEMEENTEN_FILE
...
# ✅ Gemeenten laden vanuit bundel (Contents/Resources/data/gemeenten.json)
try:
...
def start_browser():
    ...
    try:
        ...
        logging.info("✅ Chrome sessie gestart")
        return driver
    except Exception as e:
        logging.warning("⚠️ Chrome niet beschikbaar: %s", e)
        if sys.platform == "darwin":
            try:
                driver = SafariDriver()
                logging.info("✅ Safari sessie gestart")
                return driver
            except Exception as se:
                logging.error(
                    "❌ Safari kon niet starten. Zet 'Allow Remote Automation' aan."
                )
                raise se
        else:
            logging.error("❌ Geen fallback browser beschikbaar op dit platform.")
            raise e
...
def scrape_en_exporteer(
    startdatum, einddatum, maandnaam, jaar, gemeente, gemeente_code
):
        ...
        while True:
            ...
            for row in rows:
                ...
                if len(cols) >= 6:
                    ...
                    waarneming_type = (
                        "Nest - geruimd"
                        if is_nest and in_collectie
                        else "Nest - niet geruimd"
                        if is_nest
                        else "koningin"
                        if is_koningin
                        else "Individueel"
                    ...
                    gps_raw = ""
                    doublure_status = ""  # standaard leeg ivm "niet" nest meldingen
                        ...
                        try:
                            ...
                            WebDriverWait(driver_detail, 10).until(
                                EC.presence_of_element_located(
                                    (
                                        By.CSS_SELECTOR,
                                        "span.teramap-coordinates:nth-child(1)",
                                    )
                                )
                            )
                            gps_element = driver_detail.find_element(
                                By.CSS_SELECTOR, "span.teramap-coordinates:nth-child(1)"
                            )
                    ...
                    try:
                        WebDriverWait(driver_detail, 10).until(
                            EC.presence_of_element_located(
                                (
                                    By.CSS_SELECTOR,
                                    "#observation_status td.validation-status-text",
                                )
                            )
                        )
                        validatie_element = driver_detail.find_element(
                            By.CSS_SELECTOR,
                            "#observation_status td.validation-status-text",
                        )
                        validatie_status = validatie_element.text.strip()
                        print(
                            f"✅ Validatiestatus voor {waarneming_id}: {validatie_status}"
                        )
                    except Exception as e:
                        print(
                            f"⚠️ Validatiestatus niet gevonden voor {waarneming_id}: {e}"
                        )
                        validatie_status = ""
            ...
            if sessie_teller >= max_per_sessie:
                print(
                    f"🔄 {sessie_teller} meldingen bereikt — herstart Chrome-sessies."
                )
                driver.quit()
                driver_detail.quit()
                time.sleep(2)
    ...
    def vind_excelbestand(gemeente):
        ...
        if bestanden:
            ...
            return os.path.join(
                DATA_DIR, f"{gemeente}_aziatische_hoornaar_{tijdstempel}.xlsx"
            )
    ...
    def vind_kmlbestand(gemeente):
        ...
        if bestanden:
            ...
            tijdstempel = datetime.now().strftime("%Y%m%d_%H%M")
            return os.path.join(
                DATA_DIR, f"{gemeente}_aziatische_hoornaar_{tijdstempel}.kml"
            )
    ...
    # 📄 Excel-bestand aanmaken als het nog niet bestaat
    ...
    if not os.path.exists(excel_pad):
        ...
        kolommen = list(nieuwe_df.columns)
        ws.append(kolommen)
        wb.save(excel_pad)
        print(f"✅ Excel-bestand opgeslagen op: {excel_pad}")

    # 📥 Open bestaand Excel-bestand
    wb = load_workbook(excel_pad)
    ws = wb.active

    # 🧩 Bestaande Waarneming ID's verzamelen
    bestaande_ids = set()
    for row in ws.iter_rows(min_row=2, values_only=True):
        bestaande_ids.add(row[0])  # kolom A = Waarneming ID

    # 🔗 Voeg alleen nieuwe meldingen toe
    ...
    for w in waarnemingen:
        ...
        if match:
            ...
            SubElement(icon_style, "color").text = kml_colors.get(kleur, "ffff0000")
